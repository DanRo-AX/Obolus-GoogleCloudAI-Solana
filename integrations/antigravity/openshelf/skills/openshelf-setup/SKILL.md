...
Registration is also local and keeps the password out of the model transcript:

```bash
node integrations/antigravity/openshelf/runtime/server.mjs auth register --email YOU@example.com --age-confirmed
```

The runtime stores only the OpenShelf session token and short-lived query capabilities in a mode-0600 file under `~/.config/openshelf/`. It never stores an OpenShelf password or Pay private key.
...
For side-by-side role testing, launch separate shells with matching explicit selectors, for example `OPENSHELF_AGENT_PROFILE=buyer OPENSHELF_PAY_ACCOUNT=buyer agy` and `OPENSHELF_AGENT_PROFILE=contributor OPENSHELF_PAY_ACCOUNT=contributor agy`. Run the local OpenShelf login command with the same `OPENSHELF_AGENT_PROFILE` first. Profile names accept only letters, numbers, underscores, and hyphens and never change the Google login.
