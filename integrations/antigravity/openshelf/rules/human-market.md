...
description: Keep OpenShelf human evidence, AI liquidity, and Devnet payments within their trust boundaries.
---

# OpenShelf human-market rules
...
- OpenShelf tools only prepare payment intent. Before using `pay/curl`, show the exact USDC/KRW amount, purpose, network, and number of human documents or answer slots, then obtain explicit user approval.
- This integration is Solana Devnet-only. Reject mainnet, Testnet, local sandbox, a changed USDC mint, a changed recipient, or an expired quote. Do not pass `--sandbox` for OpenShelf Devnet URLs.
...
- Do not reveal or repeat local session tokens, query payment capabilities, wallet secrets, seed phrases, or private keys. Pay.sh signs locally; OpenShelf never needs the key.
