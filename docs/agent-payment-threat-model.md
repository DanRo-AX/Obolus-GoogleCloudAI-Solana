...
Status: interactive browser and Pay.sh signing enabled on Devnet; unattended
signing deliberately disabled. Phantom and Antigravity's Pay MCP path both
require local user approval, while an exact multi-document bundle or paid
open-call target requires one approval rather than one approval per
author/answer. The pure evaluator in
...
- Browser storage and the local agent session cache, which are untrusted and may
  be edited by the user or malware.
...
| Agent prompt injection | Quote preparation cannot sign; Pay invocation follows a displayed exact intent and local approval | Unapproved calls stop at `approval_required` |
| Identity confusion | Google, OpenShelf, and Pay accounts are selected and linked independently | Email equality never proves wallet ownership |
...
Interactive Pay.sh signing is not that delegation mechanism. Pay keeps the key
locally and asks for approval for a concrete call; OpenShelf receives only the
normal x402/SIWX proof. Automated or background spending remains disabled.
