...
<p class="${P}">These terms cover the OPENSHELF chat, contributor dashboard, My Memory, SHELF-1 retrieval agent, open calls, and payment functions. The current release is restricted to Solana Devnet and test assets.</p>
<h2 class="${H2}">1. Human databases and search</h2>
<p class="${P}">Accepted firsthand answers build a contributor-owned memory stream. Eligible passages become versioned documents with an anonymous handle, content hash, consent state, price, and payout address. Free search returns payment-safe metadata only. A passage is delivered only after the exact query-bound quote is paid.</p>
...
<h2 class="${H2}">2. Accounts and content</h2>
<ul class="${LIST}"><li>You must be at least 14, keep credentials secure, and provide information you are entitled to share.</li><li>Copyright remains with the contributor. You grant OPENSHELF the limited rights needed to store, index, quote, settle, and operate the service.</li><li>Private interview turns may be retained as memory context but are not sold as separate passages.</li><li>You may lock an eligible passage, append a correction, export your memory and access history, or delete the account.</li></ul>
<h2 class="${H2}">3. Existing-document payments</h2>
...
<h2 class="${H2}">5. AI liquidity</h2>
<p class="${P}">When human coverage is thin, Gemini on Vertex AI may produce a free general baseline or contributor interview prompts. AI output is not human evidence, priced inventory, Memory, authority, a paid citation, or contributor earnings. Private shelf passages are not sent to generate the baseline.</p>
<h2 class="${H2}">6. Quality, reports, and disputes</h2>
<p class="${P}">Off-topic, copied, fabricated, or generated answers may be voided. Buyers who paid for a passage may submit usefulness feedback or a report. Upheld reports can reduce reliability or lock a document. Strikes can pause auto-match, hold payouts, or suspend an account. Contributors may use the available dispute process; these controls do not guarantee accuracy.</p>
<h2 class="${H2}">7. Prohibited conduct</h2>
<ul class="${LIST}"><li>Submitting another person’s experience or generated text as your own firsthand record</li><li>Including another person’s personal information without authority</li><li>Using duplicate identities, fabricated links, or coordinated activity to manipulate ranking, calls, or settlement</li><li>Circumventing access controls, payment boundaries, rate limits, or attempting to reconstruct private shelves in bulk</li></ul>
...
<p class="${P}">Data handling is described in the <a class="${LINK}" href="/privacy">Privacy Policy</a>. OPENSHELF may change or interrupt the service. Material changes should be reviewed before continued use. These terms are governed by the law of the Republic of Korea, subject to mandatory consumer protections.</p>
</article>`
