...
<ul class="${LIST}"><li>문서 1건을 읽으려고 값을 내는 것이 열기입니다. 문서마다 가격이 다릅니다. 가격은 열기를 승인하기 전에 ₩로 표시되고, 정산은 Devnet USDC로 이뤄집니다.</li><li>이 웹사이트는 새 Phantom 메시지 서명으로 지갑 소유를 증명하고, 철회할 수 있는 선불 세션을 발급합니다. 이 세션에는 Solana 개인 키도 토큰 승인 한도도 없고, 예치된 Obolus 잔액만 잡아 둘 수 있습니다.</li><li>잔액이 모자라면 Phantom이 선택한 Devnet USDC 충전에 서명합니다. Cloud Run 작업자는 Pay.sh/MPP와 내보낼 수 없는 Google Cloud KMS 서비스 서명자를 써서 선택된 DB마다 따로 정산합니다.</li><li>결제가 끝난 확정 스냅숏만 전달됩니다. 영구적인 부분 실패가 나면 열리지 않은 최소 단위 금액이 선불 잔액으로 돌아갑니다. 남은 잔액은 인증된 지갑으로 출금할 수 있습니다.</li><li>확정된 온체인 송금은 지울 수 없습니다. 복구와 환불 기록은 멱등이며 회계를 위해 남을 수 있습니다.</li></ul>
<h2 class="${H2}">4. 공개 모집</h2>
<p class="${P}">사람이 채운 몫이 없으면 질문자가 목표 건수와 수락 답변 1건당 값을 정할 수 있습니다. 유료 모집은 Devnet USDC 승인 1회로 그 목표만큼 예치합니다. 수락된 답변은 확정된 정산 청구를 만듭니다. 취소하면 쓰지 않은 최소 단위가 원래 결제자에게 돌아갑니다. ₩0 모집은 토큰 정산이 없고 오프체인 장부를 씁니다.</p>
<h2 class="${H2}">5. 성긴 서가와 AI 기준선</h2>
<p class="${P}">사람이 채운 몫이 성기면 Vertex AI의 Gemini가 무료 일반 기준선이나 저자 인터뷰 질문을 만들 수 있습니다. AI 출력은 사람의 증거도, 값이 붙은 재고도, 기억도, 권위도, 유료 인용도, 저자 수익도 아닙니다. 기준선을 만들 때 비공개 서가의 구절은 보내지 않습니다.</p>
...
<ul class="${LIST}"><li>다른 사람의 경험이나 생성된 글을 직접 겪은 기록으로 올리는 행위</li><li>권한 없이 다른 사람의 개인 정보를 포함하는 행위</li><li>중복 신원, 조작된 링크, 조직적 활동으로 순위·모집·정산을 조작하는 행위</li><li>접근 통제, 결제 경계, 호출 제한을 우회하거나 비공개 서가를 대량으로 복원하려는 행위</li><li>서비스를 역설계하거나 허가 없이 취약점을 탐색하는 행위</li></ul>
...
<p class="${P}">서비스는 있는 그대로 제공됩니다. Obolus는 질문에 맞는 문서가 서가에 있다는 것, 인용된 구절이 정확하거나 완전하다는 것, 서비스가 중단 없이 돌아간다는 것을 보증하지 않습니다. 법이 허용하는 최대 범위에서 Obolus는 간접·특별·결과적 손해와 일실 이익에 책임지지 않습니다. Obolus의 고의나 중과실로 생긴 손해, 소비자 보호법이 정한 책임은 이 제한에서 빠집니다.</p>
...
<p class="${P}">사람이 채운 몫이 성기면 질문만 Vertex AI의 Gemini로 보내 일반 기준선을 만들 수 있습니다. 비공개 서가의 구절, 신원 정보, 지갑 주소, 결제 기록은 빠집니다. 저자 인터뷰 질문 생성에는 넓은 분야와 직접 고른 항목만 보냅니다. 유료 종합은 서버가 결제를 확인한 스냅숏만 처리합니다.</p>
...
export const AI_LIQUIDITY_PRIVACY_NOTICE_HTML_KO = `<aside class="${NOTICE}"><p class="${NOTICE_LABEL}">AI 경계</p><p class="${NOTICE_BODY}">Gemini는 일반적인 방향과 인터뷰 질문을 내놓습니다. 사람이 쓴 재고가 아닙니다. 무료 기준선 요청에는 비공개 서가의 구절이 빠집니다. 유료 종합은 서버가 결제를 확인한 스냅숏으로만 제한됩니다. AI 출력은 수익을 얻지 못하고, 사람으로 순위에 오르지 못하고, 사람 자리를 채우지 못하고, 팔리는 인용이 되지 못합니다.</p></aside>`
